...
for "_i" from 1 to _end step 1 do {
    private _d = [_pointList select _i,_pointList select 0,_pointList select _end] call fnc_pointLineDist;
...
if (_dmax > _epsilon) then {
    // resursive
    private _recResults1 = [_pointList select [0,_index + 1],_epsilon] call fnc_RDP;
    private _recResults2 = [_pointList select [_index,_end],_epsilon] call fnc_RDP;
